...
pub fn parse(input: &str) -> Result<Request, BruError> {
    ...
    let auth = parse_auth(&blocks, auth_marker.as_deref())?;

    let body = match body_marker.as_deref() {
        Some("json") => blocks.map.get("body:json").map(|s| RequestBody::Json {
            value: s.trim().to_string(),
        }),
        Some("text") => blocks.map.get("body:text").map(|s| {
            // A leading `content_type: …` line (written by `serialize`) carries
            // the body's media type; strip it off so it isn't treated as body
            // text. Falls back to `text/plain` for older files without it.
            let (content_type, value) = split_text_body(s);
            RequestBody::Text {
                value,
                content_type,
            }
...
pub fn serialize(req: &Request) -> String {
    ...
    let auth_marker = auth_mode_marker(req.auth.as_ref().unwrap_or(&Auth::None));
    ...
    match &req.body {
        ...
        Some(RequestBody::Text {
            ...
            // Persist the media type as a leading `content_type:` line so it
            // round-trips (parse reads it back via `split_text_body`).
...
/// Split a `body:text` block body into `(content_type, value)`. When the first
/// non-empty line is `content_type: …` it's consumed as the media type and the
/// remaining lines are the body; otherwise the whole block is the body and the
/// type defaults to `text/plain` (back-compat with files written before the
/// content type was persisted).
fn split_text_body(block: &str) -> (String, String) {
    let trimmed = block.trim_start();
    if let Some(rest) = trimmed.strip_prefix("content_type:") {
        // Header line: value is up to the first newline; body is what follows.
        let (ct, body) = match rest.split_once('\n') {
            Some((ct, body)) => (ct.trim().to_string(), body),
        ...
        return (ct, body.trim().to_string());
    }
    ("text/plain".to_string(), block.trim().to_string())
}
...
/// The `.bru` mode marker string for an [`Auth`] variant (e.g. `"bearer"`,
/// `"oauth2"`, `"awsv4"`). Requests carry this in their method block's
/// `auth:` line; folders carry it in an `auth { mode: … }` block.
pub fn auth_mode_marker(auth: &Auth) -> &'static str {
    match auth {
        Auth::None => "none",
        Auth::Bearer { .. } => "bearer",
        Auth::Basic { .. } => "basic",
        Auth::ApiKey { .. } => "apikey",
        Auth::OAuth2Cc { .. } => "oauth2",
        Auth::AwsSigV4 { .. } => "awsv4",
    }
}

/// Parse an [`Auth`] from already-split `.bru` blocks given the mode marker
/// (e.g. the `auth: bearer` value from a request method block, or the
/// `mode: bearer` value from a folder's `auth` block). The actual fields are
/// read from the matching `auth:<mode>` block.
///
/// Returns `Ok(Some(Auth::None))` for a missing/`none` marker, `Ok(None)` is
/// never produced here — callers that want "no auth at all" should treat a
/// missing marker as no auth themselves. Reused by both request (`bru::parse`)
/// and folder (`folder::read_one_full`) parsing so the two stay in lock-step.
pub fn parse_auth(blocks: &lexer::Blocks, mode: Option<&str>) -> Result<Option<Auth>, BruError> {
    let auth = match mode {
...
/// Serialize an [`Auth`]'s `auth:<mode> { … }` block into `out`. Does NOT
/// write the mode marker line (`auth: bearer`) — that lives in the request's
/// method block or the folder's `auth { mode: … }` block, written by the
/// caller. `Auth::None` writes nothing. Reused by request + folder serializers.
pub fn serialize_auth(auth: &Auth, out: &mut String) {
    match auth {
        Auth::None => {}
        Auth::Bearer { token } => {
